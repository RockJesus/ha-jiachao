...
async def validate_input(hass: HomeAssistant, data: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        _LOGGER.debug("Attempting login for user: %s", data[CONF_USERNAME])
        login_result = await api.login(data[CONF_USERNAME], data[CONF_PASSWORD])
        _LOGGER.debug("Login successful, result keys: %s", list(login_result.keys()) if isinstance(login_result, dict) else type(login_result))
    except Exception as err:
        _LOGGER.error("Login failed: %s: %s", type(err).__name__, err, exc_info=True)
    ...
    try:
        ...
        _LOGGER.error("Get device list failed: %s: %s", type(err).__name__, err, exc_info=True)
